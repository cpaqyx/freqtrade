...
# 服务名称
SERVICE_NAME="BTC账户1独立xql策略"
# 脚本所在目录（run目录）
SCRIPT_DIR=$(dirname "$(readlink -f "$0")")
# 项目根目录（run目录的上级就是freqtrade根目录）
PROJECT_ROOT=$(dirname "$SCRIPT_DIR")
# 配置和策略路径
ACCOUNT_DIR="user_data/btc_account1_alone_xql"
CONFIG_FILE="$ACCOUNT_DIR/config_live.json"
STRATEGY_FILE="$ACCOUNT_DIR/BTCFullPosition2_1.py"
STRATEGY_JSON="$ACCOUNT_DIR/BTCFullPosition2_1.json"
STRATEGY_NAME="BTCFullPosition2_1"
# PID文件和日志路径
PID_FILE="$SCRIPT_DIR/btc_account1_alone_xql.pid"
LOG_FILE="$PROJECT_ROOT/user_data/logs/btc_account1_alone_xql.log"
