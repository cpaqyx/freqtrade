...
class BTCFullPosition_5m(IStrategy):
    """
    BTC全仓策略
    - 日线级别
    - 只做多
    ...
    max_drop_from_high = DecimalParameter(
        0.01, 0.10, default=0.015, decimals=3, space='buy', optimize=True,
        load=True
    )
    ...
    day_rise_threshold = DecimalParameter(
        0.001, 0.10, default=0.003, decimals=3, space='buy', optimize=True,
        load=True
    )
    ...
    continuous_rise_threshold = DecimalParameter(
        0.005, 0.10, default=0.007, decimals=3, space='buy', optimize=True,
        load=True
    )
    ...
    pattern_drop_threshold = DecimalParameter(
        0.01, 0.15, default=0.015, decimals=3, space='buy', optimize=True,
        load=True
    )
    ...
    double_top_threshold = DecimalParameter(
        0.01, 0.15, default=0.03, decimals=3, space='buy', optimize=True,
        load=True
    )
    ...
    day_drop_threshold = DecimalParameter(
        0.005, 0.08, default=0.007, decimals=3, space='sell', optimize=True,
        load=True
    )
