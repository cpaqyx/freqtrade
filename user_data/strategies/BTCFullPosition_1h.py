...
class BTCFullPosition_1h(IStrategy):
    """
    BTC全仓策略
    - 日线级别
    - 只做多
    ...
    max_drop_from_high = DecimalParameter(
        0.05, 0.25, default=0.08, decimals=2, space='buy', optimize=True,
        load=True
    )
    ...
    day_rise_threshold = DecimalParameter(
        0.005, 0.10, default=0.003, decimals=2, space='buy', optimize=True,
        load=True
    )
    ...
    continuous_rise_threshold = DecimalParameter(
        0.02, 0.15, default=0.02, decimals=2, space='buy', optimize=True,
        load=True
    )
    ...
    pattern_drop_threshold = DecimalParameter(
        0.01, 0.30, default=0.03, decimals=2, space='buy', optimize=True,
        load=True
    )
    ...
    day_drop_threshold = DecimalParameter(
        0.01, 0.10, default=0.007, decimals=2, space='sell', optimize=True,
        load=True
    )
    ...
    continuous_drop_threshold = DecimalParameter(
        0.005, 0.10, default=0.007, decimals=2, space='sell', optimize=True,
        load=True
    )
    ...
    double_top_higher_threshold = DecimalParameter(
        0.005, 0.10, default=0.05, decimals=2, space='sell', optimize=True,
        load=True
    )
