...
class HighRateSowingSL5m(IStrategy):
    """
    高倍率播种策略 - 支持做空版本 (5分钟周期)
    - 分钟级别（5m）
    - 支持做多和做空（双向）
    - 逐仓模式，20倍杠杆
    - 基于波形指标的压力支撑判断
    ...
    def __init__(self, config: dict) -> None:
        ...
        logger.info("[OK] 高倍率播种策略(做空版-5m)已初始化 - 5分钟级别，20倍杠杆，逐仓模式，支持做空")
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['shape_resistance'] = np.nan
        dataframe['shape_support'] = np.nan
        dataframe['last_segment_line'] = ''
        dataframe['last_segment_min'] = np.nan
        dataframe['last_segment_max'] = np.nan
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime,
        ...
        if trade.is_short:
            ...
            if current_rate > stoploss_price:
                logger.info(f"[动态止损-空仓] {pair} 当前价{current_rate:.2f} > 止损价{stoploss_price:.2f}")
                return 0.01
        else:
            ...
            if current_rate < stoploss_price:
                logger.info(f"[动态止损-多仓] {pair} 当前价{current_rate:.2f} < 止损价{stoploss_price:.2f}")
                return 0.01
    ...
    def confirm_trade_entry(self, pair: str, order_type: str, amount: float,
        ...
        if open_count >= self.max_open_trades:
            logger.info(f"[开仓拒绝] {pair} 当前持仓{open_count}个，已达上限")
            return False
        ...
        if pair_trades:
            ...
            if existing_trade.is_short != (side == 'short'):
                logger.info(f"[开仓确认] {pair} 将平仓后开新仓")
                return True
            else:
