...
def kline_1d_shape(close_prices):
    return kline_shape(close_prices, part_cnt=5, part_percent=0.03,
                        first_part_cnt=5, first_part_percent=0.01, part_single_percent = 0.08,  debug=False)

def kline_shape(close_prices, part_cnt=5, part_percent=0.001,
                first_part_cnt=5, first_part_percent=0.0005, part_single_percent=0.002, debug=False):
    """
    基于反向遍历的K线趋势段识别算法
    从最后一根K线开始向前推移，识别趋势段
    ...
    for i in range(n - 1, 0, -1):
        ...
        if cur_part_same_percent > next_part_percent and next_part_cnt > 0:
            if debug:
                print(f"  同向力量强，吸收反向段")
            cur_part_percent = cur_part_percent - next_part_percent + cur_part_same_percent
        ...
        if len(part_list) == 0:
            # 第一段反向
            if next_part_percent > cur_part_same_percent + cur_part_percent and (
                    cur_part_cnt + next_part_cnt) < first_part_cnt:
                # print(
                #     f"第一段反向,方向：{next_part_direct}, next_part_percent:{next_part_percent}, cur_part_same_percent:{cur_part_same_percent}, "
                #     f"cur_part_percent:{cur_part_percent}, cur_part_cnt:{cur_part_cnt}, next_part_cnt:{next_part_cnt}")
                cur_part_direct = next_part_direct
                cur_part_percent = next_part_percent - cur_part_percent - cur_part_same_percent
                cur_part_same_percent = 0
                next_part_percent = 0.0
                cur_part_cnt += (next_part_cnt + cur_part_same_cnt)
                next_part_cnt = 0
        ...
        if next_part_percent - cur_part_same_percent < part_single_percent:
            if (next_part_percent - cur_part_same_percent < part_percent) or (
                    next_part_cnt + cur_part_same_cnt < part_cnt):
                continue

        # if next_part_cnt == 2:
        # print("next_part_percent:{}, cur_part_same_percent:{}, cur_part_same_cnt:{}".format(next_part_percent,
        #                                                                                     cur_part_same_percent,
        #                                                                                     cur_part_same_cnt))
        ...
        if len(part_list) == 0:
            ...
            if cur_part_cnt >= first_part_cnt or cur_part_percent >= first_part_percent:
                ...
                first_part_direct = "up"
                if start_p > end_p:
                    first_part_direct = "down"
                part = {
                    'line': first_part_direct,
                    'angle': 0,
                    'k_cnt': actual_k_cnt,
                ...
                cur_part_cnt = next_part_cnt + cur_part_same_cnt
                cur_part_percent = next_part_percent - cur_part_same_percent
...
if __name__ == '__main__':
    ...
    result = kline_shape(test_prices, debug=True)
