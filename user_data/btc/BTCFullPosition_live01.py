...
try:
    ...
    from line_format import kline_reverse_trend
...
class BTCFullPosition(IStrategy):
    ...
    max_drop_from_high = DecimalParameter(
        0.15, 0.55, default=0.36, decimals=2, space='buy', optimize=True,
        load=True
    )
    ...
    day_rise_threshold = DecimalParameter(
        0.005, 0.05, default=0.01, decimals=2, space='buy', optimize=True,
        load=True
    )
    ...
    continuous_rise_threshold = DecimalParameter(
        0.02, 0.15, default=0.06, decimals=2, space='buy', optimize=True,
        load=True
    )
    ...
    pattern_drop_threshold = DecimalParameter(
        0.01, 0.30, default=0.07, decimals=2, space='buy', optimize=True,
        load=True
    )
    ...
    double_top_threshold = DecimalParameter(
        0.03, 0.20, default=0.1, decimals=2, space='buy', optimize=True,
        load=True
    )
    ...
    day_drop_threshold = DecimalParameter(
        0.01, 0.05, default=0.02, decimals=2, space='sell', optimize=True,
        load=True
    )
    ...
    continuous_drop_threshold = DecimalParameter(
        0.005, 0.06, default=0.02, decimals=2, space='sell', optimize=True,
        load=True
    )
    ...
    double_top_higher_threshold = DecimalParameter(
        0.01, 0.12, default=0.06, decimals=2, space='sell', optimize=True,
        load=True
    )
    ...
    double_top_lower_threshold = DecimalParameter(
        0.03, 0.10, default=0.05, decimals=2, space='sell', optimize=True,
        load=True
    )
    ...
    last_down_threshold = DecimalParameter(
        0.03, 0.12, default=0.07, decimals=2, space='sell', optimize=True,
        load=True
    )
    ...
    def check_double_top(self, up_segments: list, price_a: float, segment_name: str = "") -> bool:
        ...
        if price_a >= price_b:
            ...
            if diff_higher <= self.double_top_higher_threshold.value:
                logger.info(f"[SELL] 平仓信号{segment_name}-AB: A比B高{diff_higher:.2%} (A={price_a:.2f}, B={price_b:.2f})")
                return True
        else:
            # A比B低的情况
            diff_lower = (price_b - price_a) / price_b
            if diff_lower <= self.double_top_lower_threshold.value:
                logger.info(f"[SELL] 平仓信号{segment_name}-AB: A比B低{diff_lower:.2%} (A={price_a:.2f}, B={price_b:.2f})")
                return True
        ...
        if len(up_segments) >= 2:
            ...
            if price_a >= price_c:
                ...
                if diff_higher <= self.double_top_higher_threshold.value:
                    logger.info(f"[SELL] 平仓信号{segment_name}-AC: A比C高{diff_higher:.2%} (A={price_a:.2f}, C={price_c:.2f})")
                    return True
            else:
                # A比C低的情况
                diff_lower = (price_c - price_a) / price_c
                if diff_lower <= self.double_top_lower_threshold.value:
                    logger.info(f"[SELL] 平仓信号{segment_name}-AC: A比C低{diff_lower:.2%} (A={price_a:.2f}, C={price_c:.2f})")
                    return True
